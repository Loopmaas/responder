...
func TestOk(t *testing.T) {
	...
	assert.Equal(t, loop_err.OK, int(response.code))
...
func TestUnknownError(t *testing.T) {
	...
	assert.Equal(t, loop_err.InternalServerError, int(response.code))
	assert.Nil(t, response.data)
}
...
func TestResponseToGinH(t *testing.T) {
	...
	assert.Equal(t, loop_err.CusCode(200_0000), ginH["code"])
